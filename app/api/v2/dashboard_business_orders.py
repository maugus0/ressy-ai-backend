...
Includes RBAC: admins can access all, managers can only access their business's orders.
"""
...
from app.services.business_dashboard_order_service import BusinessDashboardOrderService
...
def get_order_service() -> BusinessDashboardOrderService:
    """Dependency to get a fresh order service instance per request."""
    return BusinessDashboardOrderService()
...
def _queue_order_sms(
    ...
    """
    if not business_id or not (customer_phone and str(customer_phone).strip()):
        return
    ...
    async def _send_order_sms_notification() -> None:
        try:
            ...
            if not business:
                logger.warning("Restaurant %s not found for order SMS", business_id)
                return
...
def _check_business_access(current_user: dict, business_id: int):
    ...
    if user_type == "business":
        ...
        if int(user_business_id) != int(business_id):
            raise HTTPException(
                status_code=403,
                detail=f"You can only access orders for your own business (ID: {user_business_id})",
            )
        return
...
def _check_order_access(current_user: dict, order_id: int, order_service: BusinessDashboardOrderService):
    """
    Check if the current user has access to the specified order.
    Fetches the order and validates business access.
    ...
    """
    try:
        order = order_service.get_order_with_business_check(order_id)
    ...
    if business_id is None:
        raise HTTPException(
            status_code=500,
            detail="Order has no associated business",
        )
...
**Authorization**:
- Admins can create orders for any business
- Restaurant managers can only create orders for their own business

**Request Body**:
- `order_details` (required): List of order items. Each item must have:
  - `name` (required): Item name
  - `quantity` (required): Quantity ordered (1-100)
  - `item_id` (optional): Menu item ID for linking to menu. **If provided, the item must exist in this business's menu.**
...
**Menu Item Validation**: If `item_id` is provided in order items, the system validates that each item belongs to the specified business's menu. Business_Orders with items from other businesss will be rejected with a 400 error.
...
```
""",
    response_description="Created order with all details including order_id, business_id, user_id, status, total_amount, order_details, customization, customer information, and success message",
    ...
    responses={
            ...
            "content": {
                "application/json": {
                    "examples": {
                        ...
                        "invalid_menu_items": {
                            "summary": "Menu items don't belong to business",
                            "value": {
                                "detail": "Order contains items that don't belong to business 1: 'Margherita Pizza' (item_id 444) belongs to business 2"
                            },
                        },
                        "menu_item_not_found": {
                            "summary": "Menu item ID not found",
                            "value": {
                                "detail": "Order contains items that don't belong to business 1: item_id 999 (not found)"
                            },
                        },
                    }
                }
        ...
        403: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "You can only access orders for your own business (ID: 1)"}
                }
            },
        },
        404: {
...
async def create_order(
    ...
    order_service: BusinessDashboardOrderService = Depends(get_order_service),
...
@router.get(
    "/businesss/{business_id}/orders",
    summary="Get orders for a business (Dashboard)",
    description="""
Retrieve all orders for a specific business with optional filters.
...
**Authorization**:
- Admins can view orders for any business
- Restaurant managers can only view orders for their own business
...
""",
    ...
    responses={
        200: {
            "description": "Business_Orders retrieved successfully",
            "content": {
                "application/json": {
                    "example": {
                        "business_id": 1,
...
async def get_business_orders(
    ...
    try:
        result = order_service.get_orders_by_business(
            business_id=business_id,
            status=status,
...
- Restaurant managers can only update orders for their own business
...
- Restaurant managers can only cancel orders for their own business
...
- Restaurant managers can only delete orders for their own business
...
- Restaurant managers can only restore orders for their own business
...
async def restore_order(
    ...
    try:
        # Get business_id for the order (even if deleted)
        business_id = order_service.get_order_business_id(order_id)
        if business_id is None:
