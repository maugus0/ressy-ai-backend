...
from app.api.v2.businesses import (
    OperatingHours,
...
class ClientBusinessResponse(BaseModel):
    """Client business response - excludes sensitive integration details."""
...
class ClientUpdateBusinessRequest(BaseModel):
    """Client payload for updating a business - excludes sensitive fields."""
...
@router.get(
    "/business",
    summary="Get business (Client)",
    response_model=ClientBusinessResponse,
    description="Return the authenticated business details. Sensitive integration details are excluded.",
    response_description="Business details scoped to the authenticated business.",
    openapi_extra={
        "responses": {
            200: {
...
async def update_business(
    payload: dict = Body(..., description="Business fields to update"),
    service=Depends(get_business_service),
    claims: dict = Depends(get_current_business_user),
):
    business_id = int(claims["business_id"])
    data = validate_payload(ClientUpdateBusinessRequest, payload)
    ...
    result = service.update_business(business_id, data.model_dump(exclude_unset=True))
    return ClientBusinessResponse(**result)
...
@router.patch(
    ...
    description="Enable or disable kill switch for the authenticated business.",
    response_model=ClientBusinessResponse,
    response_description="Updated business details.",
    openapi_extra={
        "requestBody": {
        ...
        "responses": {
            200: {
                "description": "Kill switch updated for authenticated business",
                "content": {
                    "application/json": {
                        "example": {
                            "id": 10,
...
async def set_kill_switch(
    ...
    before = service.get_business(business_id)
    result = service.set_business_kill_switch(business_id, data.enabled)
    ...
    if previous_enabled != current_enabled:
        background_tasks.add_task(
            ...
            restaurant_id=business_id,  # SSE service uses restaurant_id for both
