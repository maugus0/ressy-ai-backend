...
from app.services.kill_switch_event_service import (
    ...
    emit_kill_switch_toggled_bulk_for_businesss,
)
...
class BusinessFeaturesCreate(BaseModel):
    """Feature flags for business creation (defaults to enabled)."""
...
class BusinessFeaturesUpdate(BaseModel):
    """Feature flags for business updates (all optional)."""
...
class BusinessFeaturesResponse(BaseModel):
    """Feature flags returned in business responses."""
...
class CreateBusinessRequest(BaseModel):
    ...
    name: str = Field(..., max_length=255, description="Business name (required)")
    ...
    business_type: str | None = Field(
        "restaurant", max_length=50, description="Business type (e.g., restaurant, salon, spa, clinic) - used for prompt selection"
    )
    ...
    timezone: str | None = Field(None, description="Business timezone (IANA name, e.g. America/Vancouver)")
    ...
    features: BusinessFeaturesCreate | None = Field(
...
class UpdateBusinessRequest(BaseModel):
    """Payload for updating a business (all fields optional)."""

    name: str | None = Field(None, max_length=255, description="Business name")
    address: str | None = Field(None, description="Street address")
    phone_number: str | None = Field(None, max_length=20, description="Public phone number (E.164 format)")
    twilio_phone_number: str | None = Field(
        None, max_length=20, description="Twilio phone number for routing calls (E.164 format)"
    ...
    deepgram_details: dict | None = Field(None, description="Deepgram configuration JSON")
    business_type: str | None = Field(
        None, max_length=50, description="Business type (e.g., restaurant, salon, spa, clinic) - used for prompt selection"
    )
    ...
    features: BusinessFeaturesUpdate | None = Field(None, description="Feature flags for the voice agent")
    model_config = ConfigDict(extra="ignore")
...
class BusinessResponse(BaseModel):
    ...
    features: BusinessFeaturesResponse | None = None
...
class BusinessStatsResponse(BaseModel):
    """Aggregated statistics for a business."""
    ...
    total_faqs: int = Field(..., description="FAQ entries for this business")
...
class KillSwitchSkippedBusiness(BaseModel):
    """Business skipped during bulk kill-switch enable."""
...
class KillSwitchBulkResponse(BaseModel):
    ...
    updated_count: int
    skipped_count: int
    skipped: list[KillSwitchSkippedBusiness] = Field(default_factory=list)
    model_config = ConfigDict(extra="ignore")


def get_business_service() -> BusinessService:
    """Dependency injector for BusinessService."""
    return BusinessService()


router = APIRouter(
    prefix="/api/v2/businesses",
    tags=["Businesses"],
...
@router.post(
    ...
    description="Create a new business for the Admin CRM. Admin access only.",
    response_model=BusinessResponse,
    response_description="Created business with IDs and integration metadata.",
    openapi_extra={
        "requestBody": {
            "required": True,
            "content": {
                "application/json": {
                    "schema": CreateBusinessRequest.model_json_schema(),
                    "example": {
...
async def create_business(
    payload: dict | None = Body(None, description="Business payload"),
    business_service: BusinessService = Depends(get_business_service),
):
    """
    Create a new business and return the full record.
    ...
    **Notes**:
    ...
    """
    data = validate_payload(CreateBusinessRequest, payload)
    # Use exclude_unset to allow explicit nulls to flow through for clearing values
    return business_service.create_business(data.model_dump(exclude_unset=True))


@router.get(
    "/",
    summary="List Businesss",
    description="Paginated business listing for the Admin CRM with search and credit-card filter.",
    response_model=BusinessListResponse,
    response_description="Paginated businesss with metadata.",
    openapi_extra={
        "responses": {
            200: {
                "description": "Businesss retrieved",
                "content": {
                    "application/json": {
                        "example": {
                            "items": [
...
async def list_businesss(
    ...
    is_credit_card_required: bool | None = Query(
        None, description="Filter by businesss that require a credit card for reservations"
    ),
    ...
    """
    List businesss with pagination, search, and filtering.
    ...
    **Query Parameters**:
    ...
    - `search`: Optional fuzzy search on business name
    ...
    """
    return business_service.list_businesss(
        page=page,
        limit=limit,
        search=search,
...
@router.patch(
    "/{business_id}/kill-switch",
    summary="Set Kill Switch",
    description="Enable or disable the kill switch for a specific business. Admin access only.",
    response_model=BusinessResponse,
    response_description="Updated business with kill-switch state.",
    openapi_extra={
        "requestBody": {
            "required": True,
            "content": {
...
async def set_business_kill_switch(
    business_id: int,
    ...
    before = business_service.get_business(business_id)
    result = business_service.set_business_kill_switch(business_id, data.enabled)
    ...
    if previous_enabled != current_enabled:
        background_tasks.add_task(
            emit_kill_switch_toggled,
            restaurant_id=int(result["id"]),  # SSE service uses restaurant_id for both
...
@router.patch(
    "/kill-switch/all",
    summary="Set Kill Switch For All Businesss",
    description=(
        "Bulk enable or disable kill switch for all businesss. "
        "When enabling, businesss without forwarding readiness are skipped and reported."
    ),
            ...
            200: {
                ...
                "content": {
                    "application/json": {
                        "examples": {
                            "enable_with_skips": {
                                "summary": "Enable for all, skip invalid businesss",
                                "value": {
                                    "enabled": True,
                                    "targeted_count": 3,
                                    "eligible_count": 2,
                                    ...
                                    "skipped": [
                                        {
                                            "business_id": 44,
                                            "business_name": "Downtown Kitchen",
                            ...
                            "disable_all": {
                                "summary": "Disable for all businesss",
                                "value": {
                                    "enabled": False,
                                    "targeted_count": 3,
                                    "eligible_count": 3,
...
async def set_all_businesss_kill_switch(
    background_tasks: BackgroundTasks,
    ...
    data = validate_payload(KillSwitchUpdateRequest, payload)
    result = business_service.set_all_businesss_kill_switch(data.enabled)
    changed_businesss = result.pop("_changed_businesss", [])

    if result.get("updated_count", 0) > 0:
        if changed_businesss:
            background_tasks.add_task(
                emit_kill_switch_toggled_bulk_for_businesss,
                changed_businesss=changed_businesss,
...
@router.get(
    ...
    description="Fetch a single business by ID including integration metadata. Admin access only.",
    response_model=BusinessResponse,
    response_description="Full business record for the given ID.",
    openapi_extra={
        "responses": {
            200: {
...
@router.put(
    ...
    description="Update business metadata, contact info, and integration settings. Admin access only.",
    response_model=BusinessResponse,
    response_description="Updated business record.",
    openapi_extra={
        "requestBody": {
            "required": True,
            "content": {
                "application/json": {
                    "schema": UpdateBusinessRequest.model_json_schema(),
                    "example": {
...
async def update_business(
    business_id: int,
    payload: dict | None = Body(None, description="Business fields to update"),
    business_service: BusinessService = Depends(get_business_service),
):
    """
    Update a business.
    ...
    **Notes**:
    ...
    """
    data = validate_payload(UpdateBusinessRequest, payload)
    ...
    return business_service.update_business(business_id, data.model_dump(exclude_unset=True))


@router.delete(
    "/{business_id}",
    summary="Delete Business",
    description="Permanently delete a business. Admin access only.",
...
async def delete_business(
    ...
    **Authentication**: Admin access required
    """
    return business_service.delete_business(business_id)
...
@router.get(
    ...
    response_description="Aggregate statistics for the business.",
    openapi_extra={
        "responses": {
            200: {
...
async def get_business_statistics(
    ...
    """
    Get business analytics rollups.

    **Authentication**: Admin access required
    """
    return business_service.get_business_statistics(business_id)
