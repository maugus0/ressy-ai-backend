"""
Client Analytics API for business dashboard insights.

Provides analytics endpoints for business clients to view insights about their
own business's performance, including calls, reservations, orders, menu, and Business_FAQs.

All endpoints are scoped to the authenticated business user's business.
"""
...
from app.services.business_client_analytics_service import BusinessClientAnalyticsService
...
class RestaurantAnalyticsResponse(BaseModel):
    """Comprehensive business analytics response."""
    ...
    # FAQ statistics
    total_faqs: int = Field(..., description="Total number of Business_FAQs")
...
class CallAnalyticsResponse(BaseModel):
    ...
    calls_today: int = Field(..., description="Number of calls today")
    average_call_duration: float = Field(..., description="Average call duration in seconds")
    status_breakdown: Dict[str, int] = Field(..., description="Business_Calls by status")
...
class ReservationAnalyticsResponse(BaseModel):
    ...
    total_reservations: int = Field(..., description="Total number of reservations")
    reservations_today: int = Field(..., description="Bookings for today")
...
class OrderAnalyticsResponse(BaseModel):
    ...
    total_orders: int = Field(..., description="Total number of orders")
    orders_today: int = Field(..., description="Business_Orders today")
    ...
    preparing_orders: int = Field(..., description="Business_Orders being prepared")
...
def get_analytics_service() -> BusinessClientAnalyticsService:
    """Dependency to get analytics service instance."""
    return BusinessClientAnalyticsService()
...
@router.get(
    "/analytics",
    summary="Get business analytics (Client)",
    description="""
Get comprehensive analytics for the authenticated business.

This endpoint provides a complete overview of the business's performance,
including statistics for calls, reservations, orders, menu, Business_FAQs, and recent activity.

**Authentication**: Required (business user)

**Scope**: Automatically scoped to the authenticated user's business.
...
- **FAQ statistics**: Total Business_FAQs
...
""",
    ...
    response_description="Comprehensive business analytics",
    responses={
        200: {
...
async def get_business_analytics(
    service: BusinessClientAnalyticsService = Depends(get_analytics_service),
    claims: dict = Depends(get_current_business_user),
):
    """Get comprehensive business analytics."""
    business_id = int(claims["business_id"])
    return service.get_business_analytics(business_id)
...
@router.get(
    ...
    description="""
Get detailed call analytics for the authenticated business.
...
async def get_call_analytics(
    ...
    business_id = int(claims["business_id"])
    return service.get_call_analytics(business_id)
...
@router.get(
    ...
    description="""
Get detailed reservation analytics for the authenticated business.
...
async def get_reservation_analytics(
    ...
    business_id = int(claims["business_id"])
    return service.get_reservation_analytics(business_id)
...
@router.get(
    ...
    description="""
Get detailed order analytics for the authenticated business.
...
async def get_order_analytics(
    ...
    business_id = int(claims["business_id"])
    return service.get_order_analytics(business_id)
...
@router.get(
    ...
    description="""
Get detailed menu analytics for the authenticated business.
...
async def get_menu_analytics(
    ...
    business_id = int(claims["business_id"])
    return service.get_menu_analytics(business_id)
