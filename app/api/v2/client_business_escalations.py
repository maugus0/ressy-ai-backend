...
@router.get(
    ...
    description="Retrieve paginated escalations scoped to the authenticated business with filtering and sorting.",
    response_model=EscalationListPage,
    response_description="Paginated escalations for the business.",
    openapi_extra={
        "responses": {
            200: {
...
@router.get(
    ...
    description="Retrieve details for a specific escalation scoped to the authenticated business.",
...
async def get_client_escalation(
    ...
    if str(escalation.get("business_id")) != str(business_id):
...
@router.patch(
    ...
    description="Update the status of an escalation scoped to the authenticated business. Allowed statuses: raised, forwarded, failed, resolved.",
    response_model=EscalationDetailResponse,
...
async def update_client_escalation_status(
    ...
    try:
        updated = escalation_service.update_escalation_status(
            escalation_id, data.status, business_id=str(business_id)
        )
