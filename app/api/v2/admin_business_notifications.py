...
router = APIRouter(
    prefix="/api/v2/admin",
    tags=["Admin - Business_Notifications"],
...
@router.get(
    ...
    description="List notifications across all businesss with optional filters. Use business_id to scope to one business.",
    response_model=NotificationListResponse,
)
async def list_admin_notifications(
    business_id: int | None = Query(None, description="Filter by business ID"),
    ...
    unread_count = notification_service.get_unread_count(business_id) if business_id else 0
...
@router.get(
    ...
    description="Get any notification by ID (no business scoping).",
