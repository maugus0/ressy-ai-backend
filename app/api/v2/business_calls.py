...
@router.get(
    ...
    description="Retrieve paginated call history across all businesss with filtering, sorting, and pagination.",
    response_model=CallListPage,
    response_description="Paginated calls with business metadata.",
    openapi_extra={
        "responses": {
            200: {
...
async def get_admin_calls(
    ...
    sort_by: str = Query("created_at", description="created_at, duration, business_id, started_at"),
...
@router.get(
    ...
    description="Aggregated analytics across businesss with optional business filtering and required date range.",
                ...
                "content": {
                    "application/json": {
                        "example": {
                            ...
                            "top_businesss": [{"business_id": "10", "count": 45}],
...
async def get_admin_call_analytics(
    business_id: str | None = Query(
        None,
        description="Optional business filter",
        examples={"sample": {"summary": "Restaurant ID", "value": "10"}},
    ),
    date_from: str = Query(
        ...,
    ...
    return call_service.get_dashboard_analytics(business_id=business_id, date_from=date_from, date_to=date_to)


@router.get(
    "/calls/search",
    summary="Search calls (Admin)",
    description="Search across caller phone and transcript text with optional business and date filters.",
    response_model=CallListPage,
...
async def search_admin_calls(
    ...
    business_id: str | None = Query(
        None, description="Optional business filter", examples={"sample": {"summary": "Restaurant ID", "value": "10"}}
    ),
    date_from: str | None = Query(
        None,
        description="Start date (ISO 8601)",
...
async def get_admin_call_detail(call_id: str, call_service: BusinessCallService = Depends(get_call_service)):
...
async def delete_admin_call(call_id: str, call_service: BusinessCallService = Depends(get_call_service)):
...
async def delete_admin_call_transcript(call_id: str, call_service: BusinessCallService = Depends(get_call_service)):
