...
@router.get(
    ...
    description="Retrieve paginated escalations across all restaurants with filtering, sorting, and pagination.",
    response_model=EscalationListPage,
    response_description="Paginated escalations with restaurant metadata.",
    openapi_extra={
        "responses": {
            200: {
...
async def get_admin_escalation(
    escalation_id: int, escalation_service: EscalationService = Depends(get_escalation_service)
) -> EscalationDetailResponse:
