...
class BusinessOrderCustomizationService:
    ...
    def get_effective_groups(self, catalogue_item_id: int) -> List[Dict[str, Any]]:
        groups = self.menu_repo.get_option_groups_for_item(catalogue_item_id)
        for group in groups:
    ...
    def validate_item_options(
        ...
        groups = self.get_effective_groups(catalogue_item_id)
        if not groups:
            if raw_options:
