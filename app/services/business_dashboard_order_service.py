...
class BusinessDashboardOrderService:
    ...
    def create_order(
        ...
        Raises:
            ValueError: If business not found or invalid data
        """
        ...
        # Validate order items belong to the business's menu (single batch query)
        ...
        if item_ids:
            ...
            # Check for invalid items (missing or wrong business)
            invalid_items = []
            for item_id in item_ids:
                menu_item = menu_items_by_id.get(item_id)
                if not menu_item:
                    invalid_items.append(f"item_id {item_id} (not found)")
                elif menu_item.get("business_id") != business_id:
                    item_business_id = menu_item.get("business_id")
                    item_name = menu_item.get("item_name", "Unknown")
                    invalid_items.append(
                        f"'{item_name}' (item_id {item_id}) belongs to business {item_business_id}"
                    )

            if invalid_items:
                raise ValueError(
                    f"Order contains items that don't belong to business {business_id}: "
        ...
        if customer_phone or customer_email:
            ...
            try:
                ...
                logger.warning("Failed to create user-business metadata: %s", meta_err)
    ...
    def get_order_with_business_check(self, order_id: int) -> Dict[str, Any]:
        """
        Get order by ID for RBAC checks.
        Ensures business_id is returned for access validation.

        Args:
            order_id: Order ID

        Returns:
            Order details including business_id
        ...
        # Ensure business_id is properly typed for authorization checks
        business_id = order.get("business_id")
        if business_id is not None:
            order["business_id"] = int(business_id)
    ...
    def get_orders_by_business(
        ...
        # Get orders
        orders = self.order_repo.get_orders_by_business(
            business_id=business_id,
            status=status,
            start_date=start_dt,
            end_date=end_dt,
        ...
        # Get total count
        total = self.order_repo.count_orders_by_business(
            business_id=business_id,
            status=status,
            start_date=start_dt,
            end_date=end_dt,
    ...
    def get_order_business_id(self, order_id: int) -> Optional[int]:
        """
        Get the business_id for an order (for RBAC checks).
        ...
        """
        return self.order_repo.get_order_business_id(order_id)
