"""
Service for managing menu customization option groups and values.
"""
...
class MenuOptionService:
    ...
    def __init__(
        ...
        item_option_repo: Optional[MySQLMenuItemOptionRepository] = None,
        menu_repo: Optional[MySQLMenuRepository] = None,
    ):
        self.option_repo = option_repo or MySQLMenuOptionRepository()
        self.item_option_repo = item_option_repo or MySQLMenuItemOptionRepository()
    ...
    def update_option_group_for_restaurant(
        self, restaurant_id: int, group_id: int, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        self.get_option_group_for_restaurant(restaurant_id, group_id)
    ...
    def get_option_group_for_restaurant(self, restaurant_id: int, group_id: int) -> Dict[str, Any]:
        ...
        if group.get("restaurant_id") != restaurant_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option group not found")
        return group
    ...
    def delete_option_group(self, group_id: int) -> Dict[str, Any]:
        ...
        if not existing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option group not found")
        if self.item_option_repo.has_group_attachment(group_id):
            self._bad_request("Option group cannot be deleted while attached to menu items")
    ...
    def delete_option_group_for_restaurant(self, restaurant_id: int, group_id: int) -> Dict[str, Any]:
        self.get_option_group_for_restaurant(restaurant_id, group_id)
    ...
    def create_option_value_for_restaurant(
        self, restaurant_id: int, group_id: int, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        self.get_option_group_for_restaurant(restaurant_id, group_id)
    ...
    def update_option_value_for_restaurant(
        self, restaurant_id: int, value_id: int, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        existing = self.option_repo.get_value_by_id(value_id)
        if not existing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option value not found")
        self.get_option_group_for_restaurant(restaurant_id, existing["group_id"])
    ...
    def delete_option_value_for_restaurant(self, restaurant_id: int, value_id: int) -> Dict[str, Any]:
        existing = self.option_repo.get_value_by_id(value_id)
        if not existing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option value not found")
        self.get_option_group_for_restaurant(restaurant_id, existing["group_id"])
    ...
    def attach_group_to_item(self, menu_item_id: int, group_id: int, overrides: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not group:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option group not found")
        if menu_item.get("restaurant_id") != group.get("restaurant_id"):
            raise HTTPException(
    ...
    def attach_group_to_item_for_restaurant(
        self, restaurant_id: int, menu_item_id: int, group_id: int, overrides: Dict[str, Any]
    ) -> Dict[str, Any]:
        menu_item = self.menu_repo.get_by_id(menu_item_id)
        if not menu_item or menu_item.get("restaurant_id") != restaurant_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Menu item not found")
        group = self.option_repo.get_group_by_id(group_id)
        if not group or group.get("restaurant_id") != restaurant_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option group not found")
        return self.attach_group_to_item(menu_item_id, group_id, overrides)

    def detach_group_from_item(self, menu_item_id: int, group_id: int) -> Dict[str, Any]:
        affected = self.item_option_repo.detach_item_group(menu_item_id, group_id)
        if affected == 0:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item-group mapping not found")
        return {"message": "Option group detached", "menu_item_id": menu_item_id, "group_id": group_id}

    def detach_group_from_item_for_restaurant(
        self, restaurant_id: int, menu_item_id: int, group_id: int
    ) -> Dict[str, Any]:
        menu_item = self.menu_repo.get_by_id(menu_item_id)
        if not menu_item or menu_item.get("restaurant_id") != restaurant_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Menu item not found")
        ...
        if not group or group.get("restaurant_id") != restaurant_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Option group not found")
        return self.detach_group_from_item(menu_item_id, group_id)

    def list_item_groups(self, menu_item_id: int) -> List[Dict[str, Any]]:
        return self.item_option_repo.list_item_groups(menu_item_id)
