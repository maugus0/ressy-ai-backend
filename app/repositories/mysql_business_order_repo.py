...
class MySQLOrderRepository(MySQLBaseRepository):
    ...
    def create_order(self, user_id: int, order_data: Dict) -> int:
        ...
        query = """
            INSERT INTO Business_Orders (
                user_id, business_id, status, total_amount,
    ...
    def get_latest_order_by_user(self, user_id: int, business_id: Optional[int] = None) -> Dict:
        """Fetch the most recent order for a user, optionally scoped to a business."""
        query = """
            SELECT
                id,
                user_id,
    ...
    def get_order_by_id_with_verification(self, order_id: int, business_id: int, user_id: int) -> Dict:
        """
        Get order by ID with verification that it belongs to the specified business and user.
        This ensures security by requiring all three parameters to match.

        Args:
            order_id: Order ID
            business_id: Restaurant ID (must match)
        ...
        query = """
            ...
            LIMIT 1
        """
        results = self._execute_query(query, (order_id, business_id, user_id))
        return results[0] if results else {}

    def get_order_with_user(self, order_id: int) -> Dict:
        """
        Get order with user details joined.
        Returns business_id for RBAC checks.
        """
        query = """
            SELECT
                o.id,
    ...
    def update_order_details(
        ...
        query = f"UPDATE Business_Orders SET {', '.join(fields)} WHERE id = %s"
        return self._execute_update(query, tuple(params))

    def create_order_details(self, order_id: int, catalogue_item_id: int) -> int:
        """
        Create order detail entry.
        """
        query = """
            INSERT INTO Business_Order_Details (order_id, catalogue_item_id, created_at, updated_at)
            VALUES (%s, %s, NOW(), NOW())
        """
        return self._execute_insert(query, (order_id, catalogue_item_id))
    ...
    def count_orders_by_business(
        ...
        """Count orders for a business with optional filters."""
        query = """
    ...
    def update_order(
        ...
        query = f"UPDATE Business_Orders SET {', '.join(fields)} WHERE id = %s AND deleted_at IS NULL"
    ...
    def get_order_business_id(self, order_id: int) -> Optional[int]:
        """Get just the business_id for an order (for RBAC checks)."""
        query = """
            SELECT business_id
            FROM Business_Orders
            WHERE id = %s
    ...
    def calculate_revenue_by_business(
        self,
        business_id: int,
        status: Optional[str] = None,
        start_date: Optional[datetime] = None,
        ...
        """
        Calculate total revenue for a business using SQL SUM aggregation.
