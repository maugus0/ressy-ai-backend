...
class DatabasePool:
    ...
    def _create_pool(self) -> None:
        ...
        try:
            ...
            print(f"[DB Pool] Created connection pool '{pool_name}' with size {pool_size}")
            if _should_log_connections():
                print(f"[DB Pool] Connection logging ENABLED (DB_POOL_LOG_CONNECTIONS=true)")
        except Error as e:
            error_msg = f"[DB Pool] Error creating connection pool: {e}"
            print(error_msg)
    ...
    def _log_connection_borrowed(self, conn_id: int) -> None:
        ...
        print(
            f"[DB Pool] BORROW conn_id={conn_id} thread={thread_id} "
            f"active={active}/{pool_size} total_served={total}"
        )
    ...
    def _log_connection_returned(self, conn_id: int, duration_ms: float) -> None:
        ...
        pool_size = self._pool.pool_size if self._pool else 0
        print(
            f"[DB Pool] RETURN conn_id={conn_id} thread={thread_id} "
            f"active={active}/{pool_size} held_for={duration_ms:.1f}ms"
        )
    ...
    def get_connection(self) -> Optional[PooledMySQLConnection]:
        ...
        try:
            ...
            print(f"[DB Pool] Error getting connection from pool: {e}")
            if os.getenv("ALLOW_DB_FAILURE", "false").lower() == "true":
                return None
            raise
    ...
    def close_pool(self) -> None:
        ...
        with DatabasePool._lock:
            if _should_log_connections():
                print(
                    f"[DB Pool] Closing pool. Stats: total_served={DatabasePool._total_connections_served}"
                )
            self._pool = None
