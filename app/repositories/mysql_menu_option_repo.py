...
class MySQLMenuOptionRepository(MySQLBaseRepository):
    ...
    def list_groups_by_restaurant(self, restaurant_id: int) -> List[Dict[str, Any]]:
        query = """
            SELECT * FROM Menu_Option_Groups
    ...
    def list_groups_with_values(self, restaurant_id: int) -> List[Dict[str, Any]]:
        groups = self.list_groups_by_restaurant(restaurant_id)
        if not groups:
            return []
