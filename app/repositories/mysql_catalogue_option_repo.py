"""
MySQL repositories for catalogue option groups and values.
"""
...
class MySQLCatalogueOptionRepository(MySQLBaseRepository):
    """Repository for catalogue option group/value data access."""

    def create_group(self, business_id: int, data: Dict[str, Any]) -> int:
        query = """
            INSERT INTO Catalogue_Option_Groups (
                business_id, name, description, selection_type, min_select, max_select,
    ...
    def update_group(self, group_id: int, data: Dict[str, Any]) -> int:
        ...
        query = f"UPDATE Catalogue_Option_Groups SET {', '.join(fields)} WHERE id = %s"
        return self._execute_update(query, tuple(params))

    def get_group_by_id(self, group_id: int) -> Optional[Dict[str, Any]]:
        query = "SELECT * FROM Catalogue_Option_Groups WHERE id = %s LIMIT 1"
        results = self._execute_query(query, (group_id,))
        return results[0] if results else None

    def list_groups_by_business(self, business_id: int) -> List[Dict[str, Any]]:
        query = """
            SELECT * FROM Catalogue_Option_Groups
            WHERE business_id = %s
            ORDER BY sort_order, name
        """
        return self._execute_query(query, (business_id,))

    def delete_group(self, group_id: int) -> int:
        query = "DELETE FROM Catalogue_Option_Groups WHERE id = %s"
        return self._execute_update(query, (group_id,))

    def unset_default_in_group(self, group_id: int) -> int:
        query = "UPDATE Catalogue_Option_Values SET is_default = FALSE WHERE group_id = %s"
        return self._execute_update(query, (group_id,))

    def create_value(self, group_id: int, data: Dict[str, Any]) -> int:
        query = """
            INSERT INTO Catalogue_Option_Values (
    ...
    def update_value(self, value_id: int, data: Dict[str, Any]) -> int:
        fields: List[str] = []
        params: List[Any] = []
        for field in ("name", "price_delta", "is_default", "is_available", "sort_order"):
            if field in data:
                fields.append(f"{field} = %s")
                params.append(data[field])
        if not fields:
        ...
        query = f"UPDATE Catalogue_Option_Values SET {', '.join(fields)} WHERE id = %s"
        return self._execute_update(query, tuple(params))

    def get_value_by_id(self, value_id: int) -> Optional[Dict[str, Any]]:
        query = "SELECT * FROM Catalogue_Option_Values WHERE id = %s LIMIT 1"
        results = self._execute_query(query, (value_id,))
        return results[0] if results else None

    def list_values_by_group(self, group_id: int) -> List[Dict[str, Any]]:
        query = """
    ...
    def has_order_usage_for_group(self, group_id: int) -> bool:
        query = """
            ...
            JOIN Catalogue_Option_Values mov ON mov.id = oio.option_value_id
    ...
    def delete_value(self, value_id: int) -> int:
        query = "DELETE FROM Catalogue_Option_Values WHERE id = %s"
        return self._execute_update(query, (value_id,))

    def list_groups_with_values(self, business_id: int) -> List[Dict[str, Any]]:
        groups = self.list_groups_by_business(business_id)
        if not groups:
            return []
