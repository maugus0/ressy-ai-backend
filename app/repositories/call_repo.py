...
from app.repositories.mock_data import MOCK_DATA, clone, now_iso
...
class CallRepository(BaseRepository):
    ...
    def create_call_session(
        self, user_id: str, twilio_sid: str, deepgram_session_id: str, restaurant_id: str = None
    ) -> str:
        ...
        if self.use_mock:
            MOCK_DATA["calls"][call_id] = {**item, "cost": float(item["cost"])}
            MOCK_DATA["transcripts"].setdefault(call_id, [])
            return call_id
    ...
    def update_call_cost(self, call_id: str, duration_seconds: int) -> Decimal:
        ...
        if self.use_mock:
            call = MOCK_DATA["calls"].get(call_id)
            if call:
                call["call_duration"] = duration_seconds
                call["cost"] = float(total_cost)
    ...
    def get_user_calls(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        if self.use_mock:
            items = [clone(call) for call in MOCK_DATA["calls"].values() if call.get("user_id") == user_id]
    ...
    def get_calls_by_restaurant(self, restaurant_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        if self.use_mock:
            items = [clone(call) for call in MOCK_DATA["calls"].values() if call.get("restaurant_id") == restaurant_id]
            items.sort(key=lambda x: x.get("started_at", ""), reverse=True)
            return items[:limit]
        try:
            response = self._with_retries(
    ...
    def get_all_calls(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Get all calls."""
        if self.use_mock:
            items = [clone(call) for call in MOCK_DATA["calls"].values()]
            items.sort(key=lambda x: x.get("started_at", ""), reverse=True)
            return items[:limit]
        try:
    ...
    def store_transcript(self, call_id: str, message_sequence: int, speaker: str, message: str, timestamp: str) -> None:
        ...
        if self.use_mock:
            entries = MOCK_DATA["transcripts"].setdefault(call_id, [])
            entries.append(transcript_item)
            return
    ...
    def get_call_transcripts(self, call_id: str) -> List[Dict[str, Any]]:
        ...
        if self.use_mock:
            return clone(MOCK_DATA["transcripts"].get(call_id, []))
        response = self._with_retries(self.transcripts_table.query, KeyConditionExpression=Key("call_id").eq(call_id))
        return response.get("Items", [])

    def delete_transcript(self, transcript_id: str) -> None:
        """Delete a transcript."""
        if self.use_mock:
            for call_id, entries in MOCK_DATA["transcripts"].items():
                filtered = [
                    e
                    for e in entries
                    if e.get("transcript_id") != transcript_id and e.get("message_sequence") != transcript_id
                ]
                MOCK_DATA["transcripts"][call_id] = filtered
            return
        ...
        try:
            ...
            items = response.get("Items", [])
            for item in items:
                self._with_retries(self.transcripts_table.delete_item, Key={"transcript_id": transcript_id})
