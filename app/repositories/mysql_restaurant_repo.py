...
class MySQLRestaurantRepository(MySQLBaseRepository):
    ...
    def create(self, data: Dict) -> int:
        ...
        """
        restaurant_id = self._execute_insert(query, (
            data.get('name'),
            data.get('address'),
    ...
    def get_all(
        ...
        for result in results:
            if result.get('twilio_details'):
                result['twilio_details'] = json.loads(result['twilio_details']) if isinstance(result['twilio_details'], str) else result['twilio_details']
            if result.get('deepgram_details'):
                result['deepgram_details'] = json.loads(result['deepgram_details']) if isinstance(result['deepgram_details'], str) else result['deepgram_details']
            if result.get('open_table_details'):
                result['open_table_details'] = json.loads(result['open_table_details']) if isinstance(result['open_table_details'], str) else result['open_table_details']
    ...
    def get_statistics(self, restaurant_id: int) -> Dict:
        ...
        total_seconds = minutes_result[0]['total_seconds'] if minutes_result and minutes_result[0].get('total_seconds') else 0
