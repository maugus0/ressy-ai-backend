...
class MySQLBusinessCallRepository(MySQLBaseRepository):
    """Repository for call data access in MySQL."""

    logger = get_logger(__name__)

    def create_call_session(
        self, user_id: str, twilio_sid: str | None, deepgram_session_id: str | None, business_id: Optional[str] = None
    ) -> int:
        """
        ...
        query = """
            INSERT INTO Business_Calls (
                user_id, business_id, twilio_call_sid, deepgram_request_id,
        ...
        call_id = self._execute_insert(
            query, (user_id, business_id, twilio_sid, deepgram_session_id, "in_progress", "inbound", 0, 0.000000)
        )
        self.logger.info(
            "[MySQL] Created call session: call_id=%s, user_id=%s, business_id=%s",
            call_id,
            user_id,
            business_id,
        )
    ...
    def update_call_transcript(self, call_id: int, conversation: List[Dict]) -> None:
        """
        Persist the full conversation transcript JSON on the Business_Calls table.
        """
    ...
    def get_calls_by_business(self, business_id: str, limit: int = 50) -> List[Dict]:
        query = """
            SELECT c.*, u.phone_number AS caller_phone
            FROM Business_Calls c
            LEFT JOIN Users u ON u.id = CAST(c.user_id AS UNSIGNED)
            ...
            LIMIT %s
        """
        return self._execute_query(query, (business_id, limit))
    ...
    def get_call_by_id(self, call_id: int) -> Optional[Dict]:
        query = """
            SELECT c.*, r.name AS business_name, u.phone_number AS caller_phone
            FROM Business_Calls c
            LEFT JOIN Restaurants r ON r.id = CAST(c.business_id AS UNSIGNED)
            LEFT JOIN Users u ON u.id = CAST(c.user_id AS UNSIGNED)
            WHERE c.id = %s
            LIMIT 1
        """
    ...
    def delete_call(self, call_id: int) -> int:
        ...
        return self._execute_update("DELETE FROM Business_Calls WHERE id = %s", (call_id,))
    ...
    def get_call_analytics(
        ...
        top_businesss_query = f"""
            ...
            LIMIT 10
        """
        top_businesss_rows = self._execute_query(top_businesss_query, tuple(params))
        ...
        # Match by user_id (from Business_Calls.user_id to Orders.user_id) and business_id
        # Only count conversions where user_id can be cast to non-negative integer and business_id matches
        ...
        if business_id:
            conversion_where_clauses.append("c.business_id = %s")
        if date_from:
        ...
        orders_conversion_query = f"""
            ...
            INNER JOIN Orders o ON (
                ...
                AND o.business_id = CAST(c.business_id AS UNSIGNED)
        ...
        reservations_conversion_query = f"""
            ...
            INNER JOIN Slot_Bookings sb ON (
                ...
                AND sb.business_id = CAST(c.business_id AS UNSIGNED)
            )
            WHERE {conversion_where}
        """
        ...
        return {
            ...
            "top_businesss": top_businesss_rows,
